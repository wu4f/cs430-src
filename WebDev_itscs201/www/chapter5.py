...
import traceback
import random
from questions5 import generateQuestions
from collections import OrderedDict

class Chapter5(flask.views.MethodView):
    @login_required
    def get(self):
        ...
        cursor=conn.execute("SELECT count(*) from QUESTIONBANK where CHAPTER=5")
        if ((cursor.fetchone())[0]>0):
            crsr=conn.execute("SELECT NUMBER,QUESTION,STATUS,ANSWER from QUESTIONBANK where CHAPTER=5")
            for row in crsr:
            ...
            questions = quest.genChap5(flask.session['username'])
            logentry = time.asctime() + " : " + flask.request.remote_addr + " : Successfully generated chapter 5 questions.\n"
            logfile.write(logentry)
        logfile.close()
        conn.close()
        return flask.render_template('chapter5.html', questions=questions,solved=solved,answers=answers)
    ...
    def post(self):
        try:
            ...
            if 'reset' in flask.request.form:
                conn=sqlite3.connect('../db/{0}.db'.format(flask.session['username']))
                conn.execute("delete from QUESTIONBANK where CHAPTER=5")
                conn.commit()
                conn.close()
                ...
                logfilename = logpath +  flask.session['username'] + ".log"
                logfile = open(logfilename,"a")
                logentry = time.asctime() + " : " + flask.request.remote_addr + " : Chapter 5 : Resetting Questions...\n"
                logfile.write(logentry)
                logfile.close()

                return flask.redirect(flask.url_for('chapter5'))
        ...
        #There's an answer entered by student and we need to verify if its right or wrong and provide message accordingly
            conn=sqlite3.connect('../db/{0}.db'.format(flask.session['username']))
            cursor=conn.execute("SELECT ANSWER from QUESTIONBANK where CHAPTER=5 AND NUMBER='{0}'".format(winputkey))
            for row in cursor:
                answer=row[0]
        ...
        #logging some info in log files
            ...
            logfilename = logpath +  flask.session['username'] + ".log"
            logfile = open(logfilename,"a")
            logentry = time.asctime() + " : " + flask.request.remote_addr + " : Chapter5 : " + winputkey + " : " + winput + "\n"
            logfile.write(logentry)

        #Answer verification against database answers
            if (answer== winput or answer == winput.upper()):
                flask.flash("Good Job!!!")
                conn.execute("UPDATE QUESTIONBANK SET STATUS='SOLVED' where CHAPTER=5 AND NUMBER='{0}'".format(winputkey))
                conn.commit()
                conn.close()
